...
Original file is located at
    https://colab.research.google.com/drive/1XMwXXh5uYuDtvkWhcwoC-1nVlf2CEINJ
"""
...
dir = "/content/drive/MyDrive/BC(B)_DATASET" 
doc, k, lastLine, classnum = open("all_files.txt",'w'), 0, "", -1
...
def creat_list(path): # called inside main Function
    ...
    Step_no, lists = Step_no + 1, [[] for i in range(2)]
    with open(path) as f:
        line, k = f.readline(), 0
        while line:
            classnum = int(line.split("\t")[1]) # classnum: 1 for B, 0 for M
            lists_index = 0 if classnum == 0 else 1  
            lists[lists_index].append(line.split("\t")[0])
            line, k = f.readline(), k + 1
    l = {"Malignant": len(lists[0]), "Benign": len(lists[1])}
    print(k, l)
    f.close()
    return np.array(lists, dtype=object)

def slice_train_test(arrayD, i, K): # called inside cross_validation Function
    global Step_no 
    print(str(Step_no) + ": slice_train_test Function called.")
    Step_no += 1
    exp_image_cnt_in_test = len(arrayD) // K 
    test_start = i * exp_image_cnt_in_test
    test_end = test_start + exp_image_cnt_in_test
    test_array = arrayD[test_start : test_end]
    train_array = arrayD[ : test_start] + arrayD[test_end : ]
...
def batch_generator_confusion_matrix(all_data, all_label, batch_size, shuffle, class_num = 2): # called inside Network_Config method
    global Step_no 
...
def main():
    ...
    cross_validation(categorised_data = list, K = 10, no_of_epochs = 35, no_of_classes = 2, batch_size = 18) 
main()